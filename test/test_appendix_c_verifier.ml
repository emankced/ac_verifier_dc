...
let%test "Parse simple bool operations" =
  ...
  (let (v, _) = (interp (parse "false || false") env h) in v) === Bool(false) &&
  (let (v, _) = (interp (parse "not true") env h) in v) === Bool(false) &&
  (let (v, _) = (interp (parse "not false") env h) in v) === Bool(true)
...
let%test "Parse hard bool operations" =
  ...
  (let (v, _) = (interp (parse "not false || true") env h) in v) === Bool(true) &&
  (let (v, _) = (interp (parse "not true || true") env h) in v) === Bool(true) &&
  (let (v, _) = (interp (parse "not false && true") env h) in v) === Bool(true) &&
  (let (v, _) = (interp (parse "not (5 <= 5 == (5 == 5))") env h) in v) === Bool(false)
